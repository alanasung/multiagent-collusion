# Design Document: Detecting Collusion Between Cooperating Agents

## 1. Purpose

This document is the method contract for **Detecting Collusion Between Cooperating Agents**. It specifies what is
...
One-liner: Build monitors for covert coordination in small multi-agent LLM setups.

Hypothesis: Build monitors for covert coordination in small multi-agent LLM setups.
...
|---|---|---|
| Config schema | `collude.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `collude.configs.loader` | Hydra compose API |
| Device | `collude.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `collude.models.registry` | Pinned revisions + hardware fit |
| Hooks | `collude.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `collude.models.generation` | Chat template path recording |
| Data | `collude.data` | Synthetic items, splits, manifests |
| Evaluation | `collude.evaluation` | Metrics, failure harness, protocols |
| Ablation | `collude.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `collude.reporting` | Aggregate → MD/TeX/figures |
| Cache | `collude.cache` | Versioned artifact store |
...
`src/collude/collude/` are preserved across regeneration.
